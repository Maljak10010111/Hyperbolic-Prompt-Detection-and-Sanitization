...
datasets_path = {
    ...
    'attack': '/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/Image_gen/flag.csv',
}
...
for dataset_name, dataset_path in datasets_path.items():
    ...
    folder = f"./generated_images/{dataset_name}/test"
    ...
    for idx, row in df.iterrows():
        ...
        # tokens = tokenizer(
        #     text_query, padding="max_length", max_length=77, return_tensors="pt", truncation=True
        # ).to("cuda")
        # with torch.no_grad():
        ...
        np_image = (image[0] * 255).clip(0, 255).astype(np.uint8)
        pil_image = PILImage.fromarray(np_image)
        pil_image.save(folder + f"/generated_image_{idx}.png")
        ...
        if len(all_images) % 1000 == 0:
            print(f"Generated {len(all_images)} images so far...")
            # from list of numpy arrays to list of tensors
            all_images_tensor = [torch.from_numpy(img) for img in all_images]
            # stack tensors to a single tensor
            all_images_tensor = torch.stack(all_images_tensor).to("cuda")
            # save the tensor in a .pt
            torch.save(all_images_tensor, folder + f"/all_generated_images_NSFW_{idx}_attack.pt")
            print(f"Saved all generated images to {folder}/all_generated_images_NSFW_{idx}_attack.pt")
            all_images = []  # reset the list
    ...
    all_images_tensor = torch.stack(all_images_tensor).to("cuda")
    # save the tensor in a .pt
    torch.save(all_images_tensor, folder + f"/all_generated_images_NSFW_{idx}.pt")
    print(f"Saved all generated images to {folder}/all_generated_images_NSFW_{idx}.pt")
