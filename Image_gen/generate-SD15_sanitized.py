...
datasets_path = {
    # "visu-sanitized-TLLM": "/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/data/Visu_sanitized_prompts/Thesaurus+LLM/visu_more_neutralized_prompts_llm_paired.csv",
    # "visu-semantic-TLLM": "/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/data/Visu_sanitized_prompts/Thesaurus+LLM/visu_more_semantics_prompts_llm_paired.csv",
    # 'visu-sanitized-TWR': "/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/data/Visu_sanitized_prompts/Thesaurus+Word Removal/visu_more_neutralized_prompts_thesaurus.paired.csv",
    # 'visu-semantic-TWR': "/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/data/Visu_sanitized_prompts/Thesaurus+Word Removal/visu_more_semantics_prompts_thesaurus.paired.csv",
    'visu-WR': "/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/data/Visu_sanitized_prompts/Word Removal/visu_word_removal_paired.csv",
}
...
dataset_classes = {
    ...
    # "4chan": classes.copy(),
    # "i2p": classes.copy(),
    # "adv-MMA": classes.copy(),
    # "coco": classes.copy(),
    # "visu-sanitized-TLLM": classes.copy(),
    # "visu-semantic-TLLM": classes.copy(),
    # 'visu-sanitized-TWR': classes.copy(),
    # 'visu-semantic-TWR': classes.copy(),
    'visu-WR': classes.copy(),
}
...
for dataset_name, dataset_path in datasets_path.items():
    ...
    cos_header = ["idx", "NSFW:", "SFW:", "san:", "sd_clip_mal", "sd_clip_san", "sd_clip_ben", "classification"]
    ...
    for idx, row in df.iterrows():
        ...
        if classification is not None and classification == 0:
            text_query = row["Filtered:"]
            print(f"Original prompt classified as Malicious. Using sanitized prompt: {text_query}")
        ...
        sd_clip_san = calculate_clip_score(image, [row["Filtered:"]])
        sd_clip_ben = calculate_clip_score(image, [row["Safe:"]])

        # write the results in the csv file, the delimiter is ';'
        with open(cos_path, "a") as f:
            writer = csv.writer(f)
            writer.writerow([idx, row["NSFW:"], row["Safe:"], row["Filtered:"], sd_clip_mal, sd_clip_san, sd_clip_ben, classification if classification is not None else "N/A"])
