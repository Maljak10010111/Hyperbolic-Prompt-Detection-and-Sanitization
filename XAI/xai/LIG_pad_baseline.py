"""
    This script computes word-level attributions for malicious prompts of ViSU test dataset using Layer Integrated Gradients (LIG) and PAD as the baseline with a hyperbolic MLR model.
"""
...
output_path = "/data/imaljkovic/Diffusion-Models-Embedding-Space-Defense/XAI/xai/lig_word_attributions_pad_baseline.jsonl"
...
def is_punctuation(word):
    # Returns True if word is only punctuation (dot, comma, ...)
    return all(char in string.punctuation for char in word)

def get_token_attributions(mal_prompt):
    ...
    token_reference = TokenReferenceBase(reference_token_idx=pad_token_id)
    baseline_ids = token_reference.generate_reference(77, device=DEVICE).unsqueeze(0)
...
def get_word_attributions(mal_prompt):
    input_ids = tokenizer(mal_prompt, return_tensors='pt', padding="max_length", truncation=True, max_length=77).input_ids.to(DEVICE)
    #------------------------------------------
    # Using PAD as the baseline
    pad_token_id = tokenizer.pad_token_id
    token_reference = TokenReferenceBase(reference_token_idx=pad_token_id)
    baseline_ids = token_reference.generate_reference(77, device=DEVICE).unsqueeze(0)
    # ------------------------------------------
...
#     for pair in tqdm(data):  # tqdm for progress bar
#         nsfw = pair["nsfw"]
#         malicious_prompt = re.sub(r'\.? *$', '', nsfw)
#         try:
#             attributions = get_token_attributions(malicious_prompt)
...
with open(output_path, "w", encoding="utf-8") as outf:
    for pair in tqdm(data):  # tqdm for progress bar
        ...
        try:
            attributions = get_word_attributions(malicious_prompt)
        except Exception as e:
            print("Error!")
            attributions = []
        result = {
