...
def validate_lorentz_embedding(embedding, manifold_k, tolerance=1e-5):
    ...
    lorentz_product = pairwise_inner(x=embedding, y=embedding, curv=manifold_k.abs())
...
def add_time_component(data, manifold_k):
    expected_value = 1 / manifold_k  if manifold_k > 0 else - 1 / manifold_k  # Ensure positive curvature
    x0 = torch.sqrt(expected_value + torch.sum(data**2, dim=-1, keepdim=True))
...
def load_trained_model( model_path):
    ...
    try:
        ...
        hy_model = HySAC.from_pretrained(model_id, device=Config.DEVICE).to(Config.DEVICE)
        ...
        manifold = Lorentz(k=Config.CURVATURE_K if not 'curvature' in state_dict else saved_curvature)
        ...
        return hy_model, tokenizer, model
...
def classify_embeddings(model, embeddings, description=""):
    ...
    # add time dimension to embeddings
    embeddings = add_time_component(embeddings, model.manifold.k.item()) if embeddings.shape[-1] !=769 else embeddings
...
def process_saved_embeddings(lorentz_mlr, embeddings_base_path):
    ...
    for i in range(1, 11):
        ...
        try:
            ...
            att_gen , att = embeddings.get("N1", None)
            orig = embeddings.get("original_prompt", None)
            
            if att is not None:
                attacked_emb_list.append(att)
                is_valid, violations = validate_lorentz_embedding(att, lorentz_mlr.manifold.k)
            ...
            if orig is not None:
                ...
                is_valid, violations = validate_lorentz_embedding(orig, lorentz_mlr.manifold.k)
...
def process_dataset_embeddings(lorentz_mlr, embeddings_base_path):
    """Process saved embeddings from attack experiments"""
    print("=" * 60)
    print("PROCESSING SAVED EMBEDDINGS - FROM DATASETS")
    print("=" * 60)
    
    original_emb_list, attacked_emb_list = [], []
    
    embeddings = torch.load(embeddings_base_path, map_location=Config.DEVICE)
    print(f"\nProcessing file: {embeddings_base_path}")
    embs , categories =  [], []
    for point in embeddings:
        embs.append(point[0])

        categories.append(1 if point[1].lower() == "malicious" else 0)
    #stack into tensors
    embs = torch.stack(embs, dim=0)
    categories = torch.tensor(categories, dtype=torch.long, device=Config.DEVICE)
    print(f"\n{'='*60}")
    print("CLASSIFICATION RESULTS")
    print(f"{'='*60}")
    if embs[categories == 0].numel() > 0:
        orig_logits, orig_probabilities = classify_embeddings(lorentz_mlr, embs[categories == 0], "Benign")
        original_correct = (orig_probabilities < 0.5).sum().item()
        total_original = orig_probabilities.numel()
        original_accuracy = original_correct / total_original
        print(f"Original Accuracy (should classify as benign): {original_accuracy * 100:.2f}%")
    
    if embs[categories == 1].numel() > 0:
        att_logits, att_probabilities = classify_embeddings(lorentz_mlr, embs[categories == 1], "Malicious")
        attacked_correct = (att_probabilities >= 0.5).sum().item()
        total_attacked = att_probabilities.numel()
        attacked_accuracy = attacked_correct / total_attacked
        print(f"Attacked Accuracy (should classify as malicious): {attacked_accuracy * 100:.2f}%")
        ...
        for i, prompt in enumerate(prompts):
            ...
            try:
                ...
                with torch.no_grad():
                    ...
                    hyperbolic_embedding = add_time_component(
                        embedding, 
                        lorentz_mlr.manifold.k.item()
                    )
...
def main():
    ...
    embeddings_base_path = [
        "/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/EMBEDDINGS/hyperbolic_safe_clip/test_visu_embeddings/test_visu_embeddings.pt",
        "/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/EMBEDDINGS/hyperbolic_safe_clip/mscoco/coco.pt",
        #"/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/EMBEDDINGS/hyperbolic_safe_clip/i2p/i2p.pt", 
        "/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/EMBEDDINGS/hyperbolic_safe_clip/4chain/4chain.pt",
        "/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/EMBEDDINGS/hyperbolic_safe_clip/supervised_mma/mma.pt"
    ]
    state_dict_path = "/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/LorentzMLR/final_hyperbolic_mlr_model.pth"
    # csv_paths = ['/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/COMPOSITIONAL_ATTACK/erasing/data/unsafe-prompts4703.csv',
    #              '/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/COMPOSITIONAL_ATTACK/erasing/data/coco_30k.csv', 
    #              '/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/COMPOSITIONAL_ATTACK/HyperbolicSD/data/safe_visu_text_prompts.csv',
    #              '/mnt/ssd1/mary/Diffusion-Models-Embedding-Space-Defense/COMPOSITIONAL_ATTACK/HyperbolicSD/data/unsafe_visu_text_prompts.csv']
    
    # Setup models
    hysac_model, tokenizer, lorentz_mlr = load_trained_model(state_dict_path)
    if not hysac_model or not tokenizer or not lorentz_mlr:
        print("Failed to load trained model. Exiting.")
        return
    
    # # Process saved embeddings
    for emb_file in embeddings_base_path:
        #process_saved_embeddings(lorentz_mlr, emb_file)
        process_dataset_embeddings(lorentz_mlr, emb_file)
