...
filenames = {
    "train": {
        "ailb": "visu_dpo_train_nsfw_unfiltered_images_ailb.json",
        "leonardo": "visu_dpo_train_nsfw_unfiltered_images.json",
        "laika": "visu_dpo_train_nsfw_unfiltered_images_laika.json",
    },
    "validation": {
        "ailb": "visu_validation_dpo_nsfw_unfiltered_images_ailb.json",
        "leonardo": "visu_validation_dpo_corrected_nsfw_images.json",
    },
    "test": {
        "ailb": "visu_test_dpo_nsfw_unfiltered_images_ailb.json",
        "leonardo": "visu_test_dpo_corrected_nsfw_images.json",
    },
}
...
class ViSU(Dataset):
    def __init__(
        ...
        if "leonardo" in str(Path(__file__)):
            self.cluster = "leonardo"
        elif "ailb" in str(Path(__file__)):
            self.cluster = "ailb"
        elif "laika" in str(Path(__file__)):
            self.cluster = "laika"
        else:
            raise ValueError(
                "Cluster not recognized. Please set 'leonardo', 'ailb', or 'laika'."
            )
    ...
    def img_id_to_path(self, idx):
        f = None
        for coco_images_root in (
            self.coco_train_root,
            self.coco_val_root,
            self.coco_test_root,
        ):
    ...
    def __getitem__(self, index):
        ...
        safe_image = self.imageprocessor(images=safe_image, return_tensors="pt")[
            "pixel_values"
        ].squeeze()
        nsfw_image = Image.open(nsfw_image_path)
        nsfw_image = self.imageprocessor(images=nsfw_image, return_tensors="pt")[
            "pixel_values"
        ].squeeze()
